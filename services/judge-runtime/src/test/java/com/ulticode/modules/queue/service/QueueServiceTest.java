...
class QueueServiceTest {
        ...
        void shouldThrowExceptionWhenAddFails() {
            ...
            BusinessException exception = assertThrows(BusinessException.class,
                    () -> queueService.enqueueJudgeJob(
                            SUBMISSION_ID, PROBLEM_ID, USER_ID, LANGUAGE, CODE));
            assertEquals(QueueErrorCode.QUEUE_OPERATION_FAILED, exception.getErrorCode());
        }
    }
    ...
    class EnqueueJobTests {
        ...
        void shouldThrowExceptionForUnknownQueue() {
            ...
            assertEquals(QueueErrorCode.QUEUE_NOT_FOUND, exception.getErrorCode());
        }
    ...
    class CancelJobTests {
        ...
        void shouldThrowExceptionWhenJobNotFound() {
            ...
            when(queueInspector.getJobStatus(anyString()))
                    .thenThrow(new BusinessException(QueueErrorCode.QUEUE_JOB_NOT_FOUND, "Job not found: " + JOB_ID));

            // Act & Assert
            BusinessException exception = assertThrows(BusinessException.class,
                    () -> queueService.cancelJob(JOB_ID));
            assertEquals(QueueErrorCode.QUEUE_JOB_NOT_FOUND, exception.getErrorCode());
        }
