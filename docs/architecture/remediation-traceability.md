...
| Redis business users receive broad `+@write`; health user receives `+@all`; key patterns do not constrain database-wide commands | `P0-SEC-006` | `docker/redis/generate-users-acl.sh`, ACL policy/tests | Allowed command success; `FLUSH*`, CONFIG, SHUTDOWN, cross-key denial | `054b95e` | Locally Validated |
...
| Nacos credentials are shared and production defaults to standalone-capable configuration | `P0-SEC-008` | Compose and service registry configuration | per-service required credential variables; production standalone rejection | `b689e73` | Locally Validated; live Nacos/Dubbo smoke external |
...
| Submission backfill/cutover evidence requires resumable, idempotent, zero-unexplained-diff tooling | `P1-SUB-003` | owner migration/backfill/runbook scripts | dry-run/batch/checkpoint/resume/retry/checksum/failure export rehearsal | `73d9f78` | Locally Validated; disposable rehearsal external |
| Nightly reconciliation scans `app.submissions` instead of Submission-owner facts | `P1-SUB-004` | Submission facts contract/provider, Admin reconciliation, App legacy mapper | owner-facts integration; no legacy SQL; full/incremental and multi-runner tests | `8a521d7` | Locally Validated; owner integration external |
...
| `app-api` owns 78/106 interfaces and mixes multiple Owner/internal/migration contracts | `ARCH-CONTRACT-001`, `ARCH-DUBBO-001` | API modules/POMs/contract tests | no cycles; owner/consumer/transport/lifecycle classification; japicmp | `7743d88f` | Locally Validated; N-1 consumer drain external |
...
| Streams reliability mechanisms require full crash/replay/shutdown/schema proof | `P3-STREAM-001` | Streams adapters, Inbox, Judge/Search/Notification/App/Submission | PEL/ACK/claim/DLQ/poison/crash/duplicate/replay tests | `4ebb418`, follow-up `614d90f` | DONE; real Redis crash/reclaim runtime is BLOCKED_EXTERNAL when `REDIS_HOST` is unset |
| Fixed container names and local assumptions block two-instance operation | `P3-SCALE-001` | Compose, service discovery, health and state | two-instance registration/distribution/removal/rolling restart test | `7833227` | DONE; real production-like two-instance drill is BLOCKED_EXTERNAL |
| Stateful components are single points; truthful HA/failover profiles are absent | `P3-HA-001` | `docker-compose.ha.yml`, HA contract, runtime ACL, MeiliSearch recovery documentation | HA static/dev Compose expansion, owner/replication/Sentinel ACL and external config checks, disposable Redis restart/reconnect, and architecture/docs/shell/diff/Graphify gates pass; production promotion, peer reachability and Sentinel-aware application failover remain external | `9b7c628` | Locally Validated; production failover authority external |
| Network reachability is broader than the call graph | `P3-NET-001` | Base/dev/prod/HA/observability Compose networks, per-workload egress, network contract | exact caller/target membership, internal/public port and loopback checks, HA/observability membership, disposable allow/deny drill; production expansion/firewall/DNS/ingress external | `51efd26` | Locally Validated; production network authority external |

## Architecture, testing, review and closure

| Requirement | Task IDs | Evidence | Commit | Status |
| --- | --- | --- | --- | --- |
| Remove duplicate security implementations and prevent recurrence | `ARCH-SEC-001` | Security ArchUnit and forbidden-pattern gate | `.auto-flow/evidence/architecture-remediation-20260830/arch-sec-001.result` | `bfd1919` | Locally Validated |
