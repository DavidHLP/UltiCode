#!/bin/bash
# Go code runner for the sandbox
# Compiles and executes Go code with provided inputs

set -e

TIME_LIMIT_MS="${TIME_LIMIT_MS:-10000}"
INPUT_FILE="${INPUT_FILE:-/sandbox/input/args.json}"
CODE_FILE="${CODE_FILE:-/sandbox/code/solution.go}"
...
# Create go.mod if it doesn't exist (for single-file execution)
if [ ! -f "go.mod" ]; then
...
if ! go build -o "$OUTPUT_BIN" "$CODE_FILE" 2>compile_error.txt; then
