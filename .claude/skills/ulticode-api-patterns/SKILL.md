...
description: UltiCode 项目前后端 API 对接模式参考
metadata:
  type: reference
---
...
```json
{
  ...
  "message": "success",
  "data": {},          // 实际数据
  "traceId": "t-1717000000000"  // 请求追踪ID
}
```

### 前端解析

`request.ts` 拦截器自动解包 `Result.data`，所以前端拿到的直接就是 `data` 部分：
...
- **Console 前端**：部分 API 手动做 snake_case → camelCase 映射（见 `console/src/api/problem.ts`）
...
1. **ghost types**: Management API 文件可能定义了尚无后端端点的类型（如 `UserWarning`, `UserBan`），这些是预留的，不应删除
