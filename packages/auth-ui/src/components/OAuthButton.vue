 ...
 *
 * Consolidated across console + management:
 * - Google SVG: 4-segment Google brand colors (console's version; the full
 *   multi-color mark rather than a single-tone approximation).
 * - `:focus-visible` outline: management's a11y improvement.
 * - Gap/padding/icon-size: management's slightly larger touch target.
 *
 * The button navigates to `${VITE_API_BASE_URL}/auth/{github|google}` on
 * click. VITE_API_BASE_URL is required at runtime; missing in production
 * throws immediately rather than silently leaking a localhost redirect.
 */
