...
- `assert-legacy-objects-migrated.sh` — fail-closed pre-deploy gate: no `app.user_profiles` row may still carry `/uploads/avatars/...` and no `admin.backups` COMPLETED row may lack an `object_key`.
