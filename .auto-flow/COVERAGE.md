...
| Final route, DB grant, Compose, security, concurrency and rollback gate | SPLIT-005, SPLIT-005-env-quick, SPLIT-005-env-sandbox, SPLIT-005-retirement-authority | PARTIAL — sandbox image and services-wide IT reports are now PASS; disposable grant cutover/rollback is PASS; production route/default remains local and final runtime observation is still an external release gate |
...
| Sandbox namespace IT reports missing-image prerequisites honestly | SPLIT-005-env-sandbox | PASS — local sandbox image exists; `SandboxNamespaceIsolationIT` + `SandboxForkE2EIT` execute 12/12 assertions with 0 failures/errors/skips; services-wide `*IT` aggregate is 2,690/0/0/24 |
| Submission aggregate and judge/result outboxes have one Owner/writer | SPLIT-003, SPLIT-004, CONTRACT-007 | PARTIAL — direct backend-submission provider and App duplicate/compat removal are source-verified; disposable schema/checksum/grant rollback is PASS; production route remains local until release observation |
...
| app-api contract monolith is reduced to App-owned seams | CONTRACT-001, CONTRACT-001-COMMON, CONTRACT-004, CONTRACT-008 | PASS for the source boundary — owner matrix, App API ArchUnit, migrated FQCN/POM/import scans and affected reactor tests pass; final objective gate remains CONTRACT-008 |
...
| Submission verdict/write path is single-hop and single-writer | CONTRACT-004, CONTRACT-007 | PARTIAL — direct owner providers are registered, compat/App duplicate providers and appWriter/appFence are deleted, focused/full IT and disposable grant evidence pass; production remote route and real runtime observation remain pending |
...
| Final contract-boundary gate and docs are closed | CONTRACT-008 | Required: full affected-path verification, Compose, graphify update, review/Completion Audit, current migration guide and rollback authority record |
