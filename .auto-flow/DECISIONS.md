...
## P3-IDENTITY-001: authenticate Dubbo workloads with transport mTLS

- Context: Nacos/Dubbo discovery metadata and invocation attachments are not trusted workload identity; all six Triple workloads need independent service certificates and a caller policy matching the existing delegation audience.
...
## P3-JUDGE-001: remove production Docker socket trust

- Context: production Judge inherited host Docker socket/GID trust, while the sandbox executor already owns bounded resource/network/seccomp controls and must preserve the shared workspace/result contract.
...
## 2026-09-01: repository scope closes disposable blockers

- Context: the project owner confirmed that this is an open-source repository with no production environment and authorized short-lived isolated simulation for previously blocked runtime checks.
- Decision: treat source-controlled contract proofs and disposable Compose/DinD runs as the acceptance evidence for repository tasks. Do not require or imply production registry, traffic, certificate authority, deployment, firewall/DNS, telemetry storage, or remote-host state that does not exist.
- Decision: retire the unused Submission N-1 contracts/provider in the first intentional major artifact release (`2.0.0`), keep same-major japicmp fail-closed, derive CI revisions from the current POM metadata, and run the virtual drain/error-budget/checksum/rollback proof before deleting the compatibility surface.
- Decision: keep optional runtime scripts fail-closed: an invocation without the required disposable inputs must print `BLOCKED_EXTERNAL`; the parameterized disposable invocation is the repository completion path and must clean its temporary resources.
- Consequences: implementation checkpoint `1e69b5b5eb3a607aba5f5bb5ca7da5729da6a11a` and `blocked-external-closure-20260901.result` evidence all repository-side blocked checks as PASS. The task ledger has no remaining `blocked_by` entries; future adopters remain responsible for any deployment-specific production validation.
- Affected tasks: SVC-003-GATE, P3-SCALE-001, P3-HA-001, P3-IDENTITY-001, P3-NET-001, P3-JUDGE-001, ARCH-CONTRACT-001, ARCH-DUBBO-001, REVIEW-001, REVIEW-002, CLOSURE-001.
