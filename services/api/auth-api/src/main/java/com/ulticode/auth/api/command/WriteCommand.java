 ...
 *
 * <p>Per {@code docs/ARCHITECTURE.md}, each write Command carries:
 * <ul>
 ...
 * </ul>
 *
 * <p>Rather than duplicate those four accessors on every command, the
 * auth contract exposes this small marker interface and lets each
 * command embed the metadata via {@link #idempotency()},
 * {@link #actor()}, {@link #trace()} and {@link #commandId()}.
 *
 * <p>The shape is intentionally narrow (no business fields): concrete
 * commands add their own payloads.
 *
...
public interface WriteCommand extends Serializable {
     ...
     *         Required so the auth provider can dedupe replays.
     */
