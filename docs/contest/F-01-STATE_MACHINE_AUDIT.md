...
| 5 | `STARTED→FINISHED` (user trigger) | `ContestSchedulerServiceImpl.finishVirtualContest` (前端调) | 不直接改 participant.status（应通过 #4 的 service 路径） | 🟡 **需复核**：`finishVirtualContest` 当前是占位 stub 还是真调用 #4？见 §3.1 |

### 3.1 F-01 关注：#5 与 #4 的协调

`finishVirtualContest` 由前端 `VirtualContestTimer.vue:79` 在用户点"完成"时调用。**正确的实现**：应该也走 `autoFinishVirtualParticipants` 路径（让 scheduler 在下个 tick 结算）或直接 `bulkFinishByIds(participantId)`（**不**绕过调度）。
...
| ContestParticipantStatus 转移保护 | ✅ R3 + R6.1 覆盖；#5 (`finishVirtualContest`) **需复核**（R6.2 待办 §3.1） |
| 跨状态机不变量 | ✅ R3 覆盖 |
...
- **#5 `finishVirtualContest` 实际实现**（R6.2.1 待复核）：如果走 `updateStatus` 直写而不走 scheduler 路径，违反"auto-finish 集中调度"原则。
...
- **审计**：**需复核**具体写入路径；如果代码逻辑用"now - contest.actualStartTime"而非"now - participant.startedAt"，则虚拟赛 `timeFromStart` 错误（与真实赛时钟相同，而不是虚拟时钟）
...
- R6.2.2 实际：读 `submissionService.submit` + `contestSubmissionMapper.insert` 看 `timeFromStart` 来源
- 若发现 violation，提交 1 行修复：`timeFromStart = participant.isVirtual ? (now - participant.startedAt) : (now - contest.actualStartTime)`
...
| 行动 | 优先级 | 归属 |
|------|--------|------|
| 复核 `finishVirtualContest` 实际实现 | P0 | R6.2.1 |
| 复核 `submissionService.submit` 中 `timeFromStart` 来源 | P0 | R6.2.2 |
| 不变量 B.1 generated column | P1 | R6.5 |
| `CANCELLED` / `DRAFT` 状态机完整性 | P2 | R7 |
