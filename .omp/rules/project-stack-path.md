...
- Database: `init-db/migrations/` is canonical; migrations are immutable. Fresh installs use `baseline.sql`. Owner schemas (`auth`, `admin`, `app`, `notification`, `submission`) maintain isolated Flyway configs and privileged migration accounts.
