...
| Rollback | 保持 `APP_SUBMISSION_ROUTING_MODE=local`，停止该进程或回滚到上一已验证 artifact；数据回写用 `scripts/dev/submission-schema-cutover.sh rollback`；代码回滚先恢复上一已验证 provider artifact |
...
- 生产 Compose 必须显式提供 `SUBMISSION_DB_HOST/PORT/NAME/USER/PASSWORD` 与 Redis 连接变量；`backend-submission` 不再回落到 App 的 `DB_*` 或容器内 `localhost`。`SUBMISSION_DB_USER` 必须是已单独 provision/unlock 的 Submission owner 账号。cutover runbook 的 `SUBMISSION_APP_DB_USER` 必须是实际 App 运行账号且只有待迁移表的表级 grants；schema-wide `ALL` 会被 preflight 拒绝，避免 REVOKE 后仍可跨 Owner 写入。
