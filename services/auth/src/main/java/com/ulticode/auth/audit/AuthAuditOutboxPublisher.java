...
/** Publishes Auth audit rows to the owner-specific audit stream. */
@Component
@ConditionalOnProperty(name = "auth.audit.outbox.dispatcher.enabled",
        havingValue = "true", matchIfMissing = true)
@RequiredArgsConstructor
public class AuthAuditOutboxPublisher implements AuditOutboxPublisher<AuthAuditOutboxRecord> {
    ...
    public String publish(AuthAuditOutboxRecord record) throws Exception {
        ...
        Map<String, String> fields = IntegrationEventEnvelopeContract.auditEnvelope(
                record.getId(), IntegrationEventEnvelopeContract.AUTH_OWNER,
                objectMapper.writeValueAsString(payload));
        MapRecord<String, String, String> streamRecord = StreamRecords.mapBacked(fields)
                .withStreamKey(IntegrationEventEnvelopeContract.AUTH_AUDIT_STREAM_KEY);
