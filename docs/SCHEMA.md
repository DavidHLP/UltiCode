...
tags: [docs, governance, schema, workflow]
status: living
updated: 2026-06-21
owner: architect
---
...
> **定位**：`docs/` 是一个由 LLM **增量维护、持续累积、互相链接**的活知识库——不是「被读的静态文档」。本文件是这套工作流的**唯一 schema**：它同时定义 *理念*（三层 / 三动作）和 *静态规范*（命名 / frontmatter / 链接 / 归档）。
>
> 灵感来自 karpathy 的 *LLM Wiki* 模式（见 [§12 参考](#12--参考--references)）。karpathy 原文强调：文档是抽象的，目录结构 / 格式 / 工具都**可选、模块化、按领域实例化，不要照搬**。本 schema 就是把 llm-wiki 理念实例化成适合 UltiCode 工程项目的形态。
>
> **入口**：[[index]] 是 wiki 内容目录；[[log]] 是维护时间线；本文件是规则。

## 0. 这是什么，为什么

核心洞察：与其每次提问都从原始文档**重新检索、重新综合**（RAG），不如让 LLM **一次性把知识编译进一个持久、互链、持续维护的 wiki**，然后保持它最新。wiki 是一个**随每个新源、每次探索不断增值的复利产物**——交叉引用已在、矛盾已被标记、综述已反映读过的所有东西。

UltiCode 的 `docs/` 已是 Obsidian 双链 vault（见 [§6 链接](#6--链接)）。本 schema 把它从「手写工程文档 + ADR 一次性记录 + CODEMAPS 自动镜像」推进到：**LLM 在 ingest / query / maintain 三个动作里持续综合多源知识的活 wiki**。

> **原则**：你（人类）负责**寻源、探索、提问**；LLM 负责**总结、交叉引用、归档、记账**。人类极少手写 wiki 页——手写仅限于 ADR 的「为什么」叙事和本 schema。
...
| **wiki** 层 | `docs/` 下所有综合产物（ADR / CODEMAPS / ops / theme / entity / concept 页） | **LLM 增量维护**；人类读、寻源、提问 |
...
- `backend-spring/`、`console/`、`management/`、`shared/`、`init-db/migrations/`
- 代码是真理之源。它已被两条管道自动综合：
  - [[CODEMAPS/README]] —— 周更的架构镜像（`ecc:update-codemaps`）
  - codegraph MCP —— 实时符号图（`codegraph_context` / `codegraph_trace` / `codegraph_impact`）
- ingest 代码源 = PR 触碰的模块；**无需**手工搬进 raw。

**B. 外部源（需要你主动收集）**

- 技术文章、规范、第三方 gist（如本 schema 的灵感来源 karpathy llm-wiki）、调研笔记、线上事故复盘。
- 放进 `docs/raw/<source-slug>.md`（原文 + 你的批注）。`docs/raw/` **按需创建**：有第一个外部源要 ingest 时才建（见 [§9](#9--不要做的事)「不为组织整洁建空目录」）。

**C. 运行现象**

- Arthas / 日志 / 监控里观察到的实际行为，是代码之外的「经验真源」。
- 沉淀路径：工具向 → [[ops/README]]；认知向 → 对应 entity / concept 页的「已知行为」段。

### 1.2 wiki —— 页面类型

| 类型 | 目录 | 记什么 | 何时写 | 生命周期 |
| --- | --- | --- | --- | --- |
| ADR | `adr/` | 一个不那么显然决策的**为什么** | 决策点 | accepted 后 frozen（见 [[adr/README]]） |
| CODEMAPS | `CODEMAPS/` | 代码**现在长什么样**（镜像） | 自动，周更 | 随源覆盖 |
| ops 深读 | `ops/` | 工具 / 场景参考 | 引入新工具或场景 | living |
| theme 专题 | `theme/` | 主题系统切面 | 改前端颜色 / 字体 / 密度 | living |
| **实体页** | `entities/` | **关于某个稳定实体的一切**（跨 ADR / CODEMAPS / ops 综合） | 该实体被 ≥2 处引用、值得有「总页」 | living，随源更新 |
| **概念页** | `concepts/` | **横切概念**（exactly-once、virtual contest） | 一个 idea 跨多个模块 | living |

**实体页 vs ADR vs CODEMAPS**（关键区分）：

- **ADR** 记「为什么这么定」（一次）。
- **CODEMAPS** 记「代码现在是什么样」（自动镜像）。
- **实体页 / 概念页** 记「关于 X 的综合认知」——把散落在 ADR / CODEMAPS / ops / 代码里关于 X 的信息**综合**成一页，并随每个新源更新、标记矛盾。**这是 llm-wiki 模式的核心产物：知识编译一次、保持最新，而非每次查询重算。**
...
| 触发 | 动作 |
| --- | --- |
| 代码改动 PR | 触碰模块若有 entity / concept 页 → 更新；新模块 → 评估是否建 entity 页；满足 ADR 触发条件 → 写 ADR（[[adr/README]]）；`ecc:update-codemaps` 周更镜像 |
...
**每个 ingest 动作的四件事**（最后一项是本 schema 强化的 karpathy 约定）：
...
2. [[index]] —— 先读目录，定位相关页再 drill in（karpathy：中等规模下 index 足够，无需 embedding RAG）
3. `ctx_search` —— 已索引内容（本 wiki + 已 ingest 的外部源）
4. Obsidian backlinks / outgoing links —— 看这页连着什么
5. `grep` —— 字面量（字符串、注释、日志、迁移文件名）

回答**带引用**——指向相关笔记或字面量代码路径（规则见 [§6](#6--链接)–[§7](#7--引用代码与迁移)）。

> **关键洞察**：可复用的综合——「X 与 Y 的关系」「X 的总览」——**归档回 wiki** 作为新页（entity / concept / 对比表）。探索也要复利，不要让好答案消失在对话历史里。

### 2.3 Maintain —— 保持新鲜

- **交叉引用可解析**：所有双链都指向 vault 内真实 `.md`（校验方式见 [§6](#6--链接)）
- **`updated:` 新鲜**：触碰即改 frontmatter 日期（见 [§8](#8--更新流程)）
- **矛盾要么解决、要么显式标记**：用 Obsidian callout，不要静默并存——

  ```markdown
  > [!warning] 与 [[0008-websocket-cookie-auth]] 存在张力
  > 此处说「query token 可接受」，adr/0008 明确禁止。以 adr/0008 为准；本段待修订。
  ```

- **孤儿页**（无入链）定期审视：补链，或按 [§8](#8--更新流程) 归档

## 3. index.md 与 log.md 格式

karpathy 模式的两个顶层机制，本 vault 实例化为 [[index]] 和 [[log]]。

### 3.1 [[index]] —— content catalog

- **唯一全局目录**。每页一行：`- [[link]] 一句话 summary · status · updated`
- 按 category 分组：entity / concept / adr / codemaps / ops / theme / reference
- 对子目录（`adr/`、`ops/`、`theme/`、`CODEMAPS/`）只列**入口 + 一句话**，不展开其内部页（子域细节由各自的 `README.md` 承载——见 [§9](#9--不要做的事)「不造第二个 catalog」）
- **保留角色入口表**（「你是新贡献者 / on-call / 架构师… → 从哪开始」）和「按任务索引」——它们是 catalog 的合理组成，不是冗余
- 每次 ingest 新建 / 重命名 / 删除页时同步更新

### 3.2 [[log]] —— append-only 时间线

- 条目格式：`## [YYYY-MM-DD] ingest|maintain | Title`（可被 `grep "^## \[" log.md | tail -5` 解析）
...
- **ADR 补充篇**：`NNNNa-slug.md`（如 `0005a-rollback-drill.md`，是对主决策的演练 / 补充）
- CODEMAPS：`<area>.md`（无前缀；area 是名词；**目录名 `CODEMAPS/` 保持大写**——doc-updater agent 硬编码此路径，重命名会让自动生成写回大写）
- 顶层文档：单个 `Title-Case.md`（CONTRIBUTING、RUNBOOK、ENV、SCHEMA、README）
...
- `sources:` —— 溯源列表（raw 文件名 / 代码路径 / ADR 文件名 / 外部 URL）。让「这页认知从哪来」可审计。
- `aliases:` —— Obsidian 别名，利搜索 / graph（如实体页 `aliases: [提交, 判题结果]`）。

ADR / CODEMAPS / reference 页按需补 `sources:`，不强制。
...
| 分类 | 允许的值 |
| --- | --- |
| **文档类型** | `index`、`adr`、`runbook`、`reference`、`tutorial`、`spec`、`entity`、`concept`、`schema` |
...
| 链接到另一篇笔记的某节 | `[[RUNBOOK#4 Common Issues\|RUNBOOK §4]]`（用**真实标题文本**，不是 GitHub slug） |
| 链接到一篇 ADR | `[[0008-websocket-cookie-auth\|adr/0008]]` — **总是用文件名**而非编号，重编号后链接仍有效 |
...
**交叉校验**：所有从索引笔记（[[index]]、子目录 `README.md`、`CODEMAPS/README.md`）出发的 `[[wikilink]]` 都必须可解析为 vault 内某篇 `.md`。对每个 `[[target]]`（去掉 `#anchor` 与 `|alias`），确认 target 是某篇笔记的 basename，或 `子目录/basename` 形式。定期排查越界链接（指向 `docs/` 之外的 `../` 路径）确保它们都是有意保留的字面量。

## 7. 引用代码与迁移

> （并入 [§6 链接](#6--链接)；本节强调稳定性约束。）
...
| 每季度 | 手工核对 `adr/` 索引与 [[CODEMAPS/architecture]] 镜像表，修正偏差 |
| 被替代时 | 新 ADR 把旧 ADR 置为 `status: superseded` + `superseded_by: NNNN`；两份文件在索引中并存 |
...
> **已知 TODO**：`CODEMAPS/*` 的 frontmatter 需由 `ecc:update-codemaps` 生成器在重生成时保留（见 [[CODEMAPS/README]]「重生成注意」）。把 frontmatter 输出固化进生成器是待办项。

## 9. 不要做的事

- **不要造第二个 catalog。** [[index]] 是唯一全局目录；子目录 `README.md` 是子域目录。两者不重复——顶层 index 对子目录只列入口 + 一句话。不要在别处再维护一份「所有文档列表」。
...
- **不要手写自动生成的 `CODEMAPS/*` 内容。** 那个 `<!-- Generated: ... -->` 头就是提示：如果你这次的编辑是保持它准确的唯一动作，跑 `ecc:update-codemaps`。
- **不要把代码路径 / 迁移名 / 仓库根文件转成 `[[wikilink]]`。** 它们不是 vault 笔记，转了会在图谱里产生断链。wikilink 只用于 `docs/` 内的 `.md` 笔记之间。
- **CODEMAPS 命名特例**：`docs/CODEMAPS/` 内部用 `README.md` 作索引（本仓库历史约定）；doc-updater agent 文档里出现的 `INDEX.md` 是笔误，以实际 `README.md` 为准。

## 10. 启动状态

- 已建首批 **entity 页**：[[contest]]、[[submission]]、[[sandbox-dform]]。
- 已建首批 **concept 页**：[[exactly-once]]、[[virtual-contest]]。
- **暂缓**（信息源单薄，等专属 ADR / 实现文档出现再建）：csrf-token、refresh-token、csrf-lifecycle。
- 当前 `docs/` **没有** `raw/`——在**第一次真实外部源 ingest 需求**时创建，不预先建空目录。代码源是一等 raw，无需搬入。
...
| `ctx_search` / `ctx_batch_execute` | query 时检索已索引内容（本 wiki + 外部源），原始字节不进对话 |
| Obsidian（桌面）+ graph view | **本 vault 原生浏览器**：看 wiki 形态、哪些页是 hub、哪些是孤儿 |
| [[index]] / [[log]] | query 入口 / 维护时间线（见 [§3](#3-indexmd-与-logmd-格式)） |
| `ecc:update-codemaps` | ingest 代码改动后重生成 [[CODEMAPS/README]] 镜像（周更） |
...
- karpathy, *LLM Wiki* —— <https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f>（本 schema 的理念来源；原文强调「抽象、可选、按领域实例化」）
- [[index]] — wiki 内容目录
- [[log]] — 维护时间线
- [[adr/README]] — ADR 专属工作流
- [[ops/README]] — 运维子目录规范
- [[theme/README]] — 主题系统专题
- [[CODEMAPS/README]] — 架构镜像生成约定
