...
| `app-api` owns 78/106 interfaces and mixes multiple Owner/internal/migration contracts | `ARCH-CONTRACT-001`, `ARCH-DUBBO-001` | API modules/POMs/contract tests | no cycles; owner/consumer/transport/lifecycle classification; japicmp | `7743d88f`, `cb40a226934ec501b788a1a673fe864d41d35ae0` | Locally Validated; N-1 consumer drain external |
...
| Streams reliability mechanisms require full crash/replay/shutdown/schema proof | `P3-STREAM-001` | Streams adapters, Inbox, Judge/Search/Notification/App/Submission | PEL/ACK/claim/DLQ/poison/crash/duplicate/replay tests; real Redis 6-test crash/reclaim/dedup/DLQ run | `4ebb418`, `614d90f`, `cb40a226934ec501b788a1a673fe864d41d35ae0` | DONE; production fault injection external |
| Fixed container names and local assumptions block two-instance operation | `P3-SCALE-001` | Compose, service discovery, health and state | two-instance registration/removal/rolling restart/failure drill with distinct ports; merged config expansion | `7833227`, `cb40a226934ec501b788a1a673fe864d41d35ae0` | DONE; production multi-host rollout/traffic authority external |
| Stateful components are single points; truthful HA/failover profiles are absent | `P3-HA-001` | `docker-compose.ha.yml`, HA contract, runtime ACL, MeiliSearch recovery documentation | HA static/dev Compose expansion, owner/replication/Sentinel ACL and external config checks, disposable Redis restart/reconnect, and architecture/docs/shell/diff/Graphify gates pass; production promotion, peer reachability and Sentinel-aware application failover remain external | `9b7c628`, `cb40a226934ec501b788a1a673fe864d41d35ae0` | Locally Validated; production failover authority external |
...
| Align tasks, commits, evidence, Git and final report | `CLOSURE-001` | final task/YAML/docs/Git/verification checks | `.auto-flow/evidence/architecture-remediation-20260830/closure-001.result` + `.auto-flow/evidence/architecture-remediation-20260830/review-follow-up-20260901.result` | `fc31cf35eecbd48ee87392a3e7e0366975191c2b` | Locally Validated |
