...
user ulticode-app on #$(hash_of "$APP_REDIS_PASSWORD") resetkeys ~rate-limit:* ~userStats:* ~contestRanking:* ~contest:* ~monitoring:* ~queue:* ~judge:* ~problem:* ~email_queue ~notification_queue ~stream:integration resetchannels &ulticode:ws:broadcast $DATA_GRANTS +@pubsub +info
