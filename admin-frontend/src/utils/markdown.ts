...
const groupFencesPlugin = (md: MarkdownIt) => {
  ...
  md.renderer.rules.code_group = (tokens, idx, options) => {
    ...
    group.forEach((fence: InstanceType<typeof md.core.State.prototype.Token>, index: number) => {
    ...
    group.forEach((fence: InstanceType<typeof md.core.State.prototype.Token>, index: number) => {
      ...
      if (options.highlight) {
        highlightedCode =
          options.highlight(fence.content, langName, '') || md.utils.escapeHtml(fence.content)
      } else {
