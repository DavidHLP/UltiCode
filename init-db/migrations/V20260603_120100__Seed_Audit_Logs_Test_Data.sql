-- Seed Test Data: Audit Logs
-- ------------------------------------------------------------
-- 拆分自 V20260602_120200__Insert_Test_Data.sql (Section: Audit Logs)
-- 维护指南: 修改 / 扩展 audit_logs 测试数据时, 仅编辑本文件
--
-- 用途: 验证 PROBLEM 实体的审计日志 UI 与 API
-- 路由:  /admin/audit/log?entityType=PROBLEM&entityId=1
-- 风险:  低 (测试数据, 使用 ON DUPLICATE KEY UPDATE 保证幂等)
-- 回滚:  DELETE FROM audit_logs WHERE id LIKE 'audit-log-%';
-- ------------------------------------------------------------

-- Insert test audit logs for PROBLEM entity
-- Note: Assuming admin user exists with id 'admin-001' (from V20260602_120100)
