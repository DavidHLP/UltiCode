"""Authorized corpus path, kept separate from the pinned synthetic baseline.
...
module therefore loads a *different* corpus from ``corpus_authorized/`` and
...
DAV-58's requirement for genuinely licensed real sources. That gate stays open.
"""
...
CORPUS_DIR = Path(__file__).resolve().parents[1] / "corpus_authorized"
MANIFEST_PATH = Path(__file__).resolve().parents[1] / "data" / "authorized_manifest.json"
...
def _load_documents() -> tuple[SourceDocument, ...]:
    ...
    for path in sorted(CORPUS_DIR.glob("*.md")):
        ...
        if not text:
            raise ManifestError(f"{path.name}: empty authorized document")
        lines = len(text.splitlines())
        documents.append(
            SourceDocument(
                doc_id=f"authorized-{path.stem}",
                version="v1",
                source_path=f"services/agent/corpus_authorized/{path.name}",
    ...
    if not documents:
        raise ManifestError("authorized corpus is empty")
    return tuple(documents)


def load_authorized_corpus() -> tuple[SourceDocument, ...]:
...
def search_authorized(query: str, *, limit: int = 3) -> tuple[object, ...]:
    return keyword_search(query, limit=limit, documents=load_authorized_corpus())


def answer_with_authorized_evidence(question: str, submission: dict[str, str]) -> dict[str, object]:
    ...
    hits = tuple(
        hit
        for hit in search_authorized(question)
    ...
    return {
        ...
        "citations_verified": all_verified(
            check_citations(citations, load_authorized_corpus())
        ),
    }
