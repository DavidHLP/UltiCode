...
- 2026-08-19 (ARCHFIX-003 planning): user authorized current TEST-TARGET as the only environment. Resolved design blocker with ARCHFIX-DEC-005: add only a versioned, batched Search UserDirectoryQueryPort; keep Admin analytics and Submission admin contracts independent. Task is in_progress for implementation.

- 2026-08-19 (ARCHFIX-003 recovery): existing AdminAnalyticsPort and SubmissionAdminReadPort are already coarse contracts, while OwnerUserSearchReadAdapter still composes Auth account/identity calls with local profiles. No safe single contract scope can be implemented without a bounded design decision; ARCHFIX-003 blocked and returned to planning.

- 2026-08-19 (ARCHFIX-002 reopen and closure): added per-owner credential generation to init-env.sh and removed migrate.sh shared DB_USER fallback; fixed strict production Compose service structure; fresh owner preflight, migration, soft-delete, grant-negative and Compose evidence passed. ARCHFIX-002 closed; ARCHFIX-003 ready.

- 2026-08-19 (ARCHFIX-002 implementation): isolated Auth/Admin/App/Notification runtime datasource defaults and production Compose owner bindings; fixed Compose service-boundary regressions; owner preflight, migration, soft-delete and grant-isolation evidence passed. ARCHFIX-002 closed; ARCHFIX-003 ready.

- 2026-08-19 (ARCHFIX-001 review): inspected argument parsing, post-.env profile resolution, `--only`/`--frontend-only` interactions and fail-closed dev-full gate; no confirmed findings.

- 2026-08-19 (ARCHFIX-001 implementation): added explicit `--mode dev-lite|dev-full` to `scripts/dev/up.sh`; dev-lite forces local Submission routing and skips cutover gate, while dev-full retains remote/cutover fail-closed behavior. Focused checks: `bash -n`, `--help` option check, and isolated mode smoke passed. Task remains `in_progress` pending review.

- 2026-08-19 (ARCHFIX planning round): preserved the prior ARCH-002/ARCH-003 blocker-remediation and Submission/Search extraction packets as historical records; created active `ARCHFIX-001..006` DAG for the user-requested Services architecture hardening; mapped all findings in `.auto-flow/COVERAGE.md`; recorded ARCHFIX decisions in `.auto-flow/DECISIONS.md`; set `ARCHFIX-001` to `ready`; updated `.auto-flow/PLAN.md`, `.auto-flow/RESUME.md` and `.auto-flow/HANDOFF.yaml`; no business code changed.
...
- 2026-08-20 (recover + plan ARCHFIX-004): Reconciled current branch/worktree and preserved all seven modified .auto-flow files; no business source is dirty. Graph/source verification confirmed the SearchController -> SearchReadProjection -> DefaultSearchReadProjection -> four SearchSource chain, offset ignored by current DB adapters, and missing offset/count app-api contracts. DefaultSearchReadProjectionTest passed 15/15 with no failures/errors/skips; this is legacy behavior evidence and does not close whole-request fallback. Replanned ARCHFIX-004-001 as ready atomic contract-plus-adapters work, recorded bounded account_id ASC user merge, canonical MEILI env-name alignment for 004-003, and kept real Meili E2E blocked until an actual service is available.

- 2026-08-20 (ARCHFIX-004-001 implementation): Migrated SearchSource rows/count contract, four app-api read seams, source adapters, Solution provider/Admin compatibility, Auth usernameOnly count, App profile pagination/count and UserDirectoryQueryPort bounded account_id ASC union. Added focused contract/owner coverage; app/admin compile + test-compile and Search/owner focused suite passed 37/0/0/0. Updated migration-guide ownership note and refreshed graphify/codebase-memory; Task remains in_progress for full Review.

- 2026-08-20 (ARCHFIX-004-001 Review): Found and fixed specific-index projection offset propagation being hard-coded to zero; added page-2 regression. Recheck passed 38/0/0/0; contract/wiring tests passed including SolutionApiContractShapeTest and BackendAppApplicationTest; Review Confirmed=0. Final validation remains before Task closure.

- 2026-08-20 (ARCHFIX-004-001 Validation): Affected reactor verify passed with BUILD SUCCESS and JaCoCo PASS; Surefire aggregation reported 604 reports, 2029 tests, 0 failures, 0 errors and 16 skips. Final YAML parse, coverage refresh and git diff --check passed. Existing backup-test mysqldump error logs were expected test scenarios; no failing Surefire reports. Ready for Completion/Coverage Audit.

- 2026-08-20 (ARCHFIX-004-001 Completion/Coverage Audit): All acceptance and coverage rows closed with fresh evidence, Review Confirmed=0, compatibility/rollback assumptions intact, and user worktree changes protected. Task set to done; ARCHFIX-004-002 set ready. No commit/push/release/deploy authorized.

- 2026-08-20 (ARCHFIX-004-002 implementation/review/validation): DefaultSearchReadProjection now computes exact DB totals, maps global offsets across fixed source order and returns stable out-of-range pages. Added cross-source and boundary regressions. Focused suite passed 40/0/0/0; affected reactor verify passed 2031/0/0/16 with BUILD SUCCESS and JaCoCo PASS; Review Confirmed=0. Ready for Completion/Coverage Audit; Meili path remains deferred to ARCHFIX-004-003/004.

- 2026-08-20 (ARCHFIX-004-002 Completion/Coverage Audit): Acceptance, Coverage, Review and Tier-C Validation evidence reconciled; Task set to done and ARCHFIX-004-003 set ready. No commit/push/release/deploy authorized.

- 2026-08-20 (ARCHFIX-004-003 implementation/review/validation): Meili reads now use SDK reported totals, propagate index failures to whole-request DB fallback, and align App config with MEILI_HOST/MEILI_MASTER_KEY. Focused suite 40/0/0/0, affected reactor verify 2031/0/0/16 with BUILD SUCCESS/JaCoCo PASS, Compose dev/prod config with ephemeral key pass, Review Confirmed=0. Task closed; ARCHFIX-004-004 real E2E set ready.

- 2026-08-20 (ARCHFIX-004-004 real E2E): Pulled getmeili/meilisearch:v1.8, ran a loopback disposable service, and passed DefaultSearchReadProjectionRealMeiliIT 2/2 with real SDK/index writes, specific/all totals/offsets and broken-client whole-request fallback. Run-unique documents were cleaned; container stopped; Compose network overlap was not force-repaired. Task closed; ARCHFIX-005 set ready.

- 2026-08-20 (ARCHFIX-005 blocker inventory): Read current production Compose, Submission route/cutover config, rollback comments and DEV-LOCAL observation scripts. External route/grant authority, all-writer quiesce, remote stability/observation window, rollback/reconciliation sign-off and retirement authority are absent; local rehearsal is explicitly non-production. Preserved all compatibility/shadow paths and marked ARCHFIX-005 blocked; ARCHFIX-006 remains pending.

- 2026-08-20 (ARCHFIX-005 authority override): User confirmed the project has no production environment and authorized reversible blocker remediation in the sole development/TEST-TARGET. Reopened ARCHFIX-005 as in progress; production-authority wording no longer blocks development execution. No push, publish, applied-migration edit or destructive data operation authorized.

- 2026-08-20 (ARCHFIX-005 development closure): Fixed Compose network allocation, in-memory Meili key fallback, cutover account defaults and misleading expected-failure logging. Aligned App/Judge queue flags across dev-lite/dev-full. Disposable owner integration passed Submission cutover copy, App grant revoke, rollback copy-back and grant restore; focused route/provider 22/0/0/0, preflight regression, quick gate, Compose, shell/Node/YAML/diff and graph refresh passed. Closed ARCHFIX-005 as runtime retirement with rollback seams preserved; ARCHFIX-006 is the active final development gate.

- 2026-08-20 (ARCHFIX-006 closure): Quick gate and final control-plane audit passed. ARCHFIX-001..006 are closed for the sole development/TEST-TARGET; no production environment, release, push, publish or destructive operation was performed.

- 2026-08-20 (CRFIX-001 closure): Closed all four Search review findings plus review-loop performance, boundary-validation, offset-overflow and Meili cleanup findings. Focused, real MySQL collation, real Meili v1.8, standard wrapper integration, documented bare `*IT`, affected reactor verify, JaCoCo, graph/coverage and diff gates passed. No commit, push, publish or production action performed.
- 2026-08-20 (CRFIX-SEC-001 closure and control-plane reconciliation): Review confirmed public `/search` resource amplification through cap-triggered database fallback and cross-owner exact counting. Added the existing `@RateLimit` boundary (`search:read`, 60/min), raised query minimum length to 2, and added `SearchQueryDTOValidationTest`; final focused Search suite passed 39/0/0/0. Re-ran `env -u MEILI_E2E_HOST -u MEILI_E2E_KEY ./scripts/dev/test.sh integration` after these fixes; all integration checks passed. Updated HANDOFF, RESUME, TASKS and COVERAGE with CRFIX-SEC-001, final evidence, and HEAD `228d6e05340944ba7e0952541ade4e6606934566`; no push, publish, deployment or production action authorized.
