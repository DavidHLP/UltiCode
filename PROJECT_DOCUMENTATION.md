...
- 同一 HA profile 提供 Redis replica 与 `redis-sentinel-1/2/3`，仍挂载运行时生成的按 Owner ACL（`default off`、Auth/Admin/App/Submission/Search/Notification/Judge/Ops/Health 身份不变）。Sentinel/replica 配置必须来自 `REDIS_HA_CONFIG_DIR` 的 secret-managed 文件；只有配置真实复制、quorum=2、单次故障转移并让客户端使用 Sentinel-aware URI 后，Redis 才有数据平面 failover。当前 Spring Data/Redisson 默认仍是单 endpoint，不能仅启动 profile 就声称应用无感切换。
