...
call() {
  local method="$1" path="$2" body="${3:-}" out="${4:-/tmp/uc-body.json}"
  : > "$HEADERS_JAR"
  local code
  if [ -n "$body" ]; then
...
curl -sS -c "$COOKIE_JAR" -o /tmp/uc-login.json -w 'HTTP=%{http_code}\n' \
  -X POST "$BASE/auth/login" \
  -H 'Content-Type: application/json' \
  --data-binary '{"username":"admin","password":"admin123"}'

CSRF=$(python3 -c "import json; d=json.load(open('/tmp/uc-login.json')); print(d.get('data',{}).get('csrfToken',''))" 2>/dev/null || echo "")
...
code=$(call GET "/admin/solutions?page=1&limit=5" "" /tmp/uc-1.json)
log "list" "$([ "$code" = "200" ] && echo OK || echo FAIL)" "code=$code"
python3 -c "import json; d=json.load(open('/tmp/uc-1.json')); print('total=',d.get('data',{}).get('total'),'items=',len(d.get('data',{}).get('items',[])))"
python3 -c "import json; d=json.load(open('/tmp/uc-1.json')); it=d.get('data',{}).get('items',[]); print('first=',json.dumps(it[0],ensure_ascii=False,default=str) if it else None)"

# 1b. isFlagged filter
code=$(call GET "/admin/solutions?isFlagged=true&page=1&limit=5" "" /tmp/uc-1b.json)
log "list-flagged-filter" "$([ "$code" = "200" ] && echo OK || echo FAIL)" "code=$code"
python3 -c "import json; d=json.load(open('/tmp/uc-1b.json')); print('flaggedTotal=',d.get('data',{}).get('total'),'items=',len(d.get('data',{}).get('items',[])))"

# 1c. sortBy=views desc
code=$(call GET "/admin/solutions?sortBy=views&sortOrder=desc&page=1&limit=3" "" /tmp/uc-1c.json)
log "list-sort-views" "$([ "$code" = "200" ] && echo OK || echo FAIL)" "code=$code"
python3 -c "import json; d=json.load(open('/tmp/uc-1c.json')); it=d.get('data',{}).get('items',[]); print('viewsOrder=',[i.get('views') for i in it])"

# 1d. search
code=$(call GET "/admin/solutions?search=two&page=1&limit=3" "" /tmp/uc-1d.json)
log "list-search" "$([ "$code" = "200" ] && echo OK || echo FAIL)" "code=$code"

# 1e. isDeleted=true (deleted-only view)
code=$(call GET "/admin/solutions?isDeleted=true&page=1&limit=5" "" /tmp/uc-1e.json)
log "list-deleted" "$([ "$code" = "200" ] && echo OK || echo FAIL)" "code=$code"
python3 -c "import json; d=json.load(open('/tmp/uc-1e.json')); print('deletedTotal=',d.get('data',{}).get('total'),'items=',len(d.get('data',{}).get('items',[])))"

# ===== 2. GET /admin/solutions/flagged =====
echo "$SEP 2. GET /admin/solutions/flagged $SEP"
code=$(call GET "/admin/solutions/flagged?page=1&limit=5" "" /tmp/uc-2.json)
log "list-flagged-endpoint" "$([ "$code" = "200" ] && echo OK || echo FAIL)" "code=$code"
python3 -c "import json; d=json.load(open('/tmp/uc-2.json')); print('keys=',list(d.keys()),'flaggedTotal=',d.get('data',{}).get('total'),'items=',len(d.get('data',{}).get('items',[])))"

# ===== 3. GET /admin/solutions/{id} =====
echo "$SEP 3. GET /admin/solutions/{id} $SEP"
code=$(call GET "/admin/solutions/sol-s-001" "" /tmp/uc-3.json)
log "detail-existing" "$([ "$code" = "200" ] && echo OK || echo FAIL)" "code=$code"
python3 -c "import json; d=json.load(open('/tmp/uc-3.json')); v=d.get('data') or {}; print('voKeys=',list(v.keys()),'isFlagged=',v.get('isFlagged'),'flaggedReason=',v.get('flaggedReason'),'publishedBy=',v.get('publishedBy'),'deletedBy=',v.get('deletedBy'))"

code=$(call GET "/admin/solutions/sol-nope-999" "" /tmp/uc-3b.json)
# BUG-Q1: error code renumbered 50001 → 50401 (no longer collides with DATABASE_ERROR).
log "detail-missing" "$([ "$code" = "404" ] && grep -q '"code":50401' /tmp/uc-3b.json && echo OK || echo NOTE)" "code=$code body=$(head -c200 /tmp/uc-3b.json)"

# ===== 4. POST /admin/solutions/{id}/flag =====
echo "$SEP 4. POST /admin/solutions/sol-s-002/flag $SEP"
code=$(call POST "/admin/solutions/sol-s-002/flag" '{"reason":"contains PII"}' /tmp/uc-4.json)
refresh_csrf
log "flag-success" "$([ "$code" = "200" ] && echo OK || echo FAIL)" "code=$code newCsrfLen=${#CSRF}"
python3 -c "import json; d=json.load(open('/tmp/uc-4.json')); v=d.get('data') or {}; print('isFlagged=',v.get('isFlagged'),'flaggedReason=',v.get('flaggedReason'),'flaggedAt=',v.get('flaggedAt'))"

# 4b. flag with empty reason (validation)
code=$(call POST "/admin/solutions/sol-s-003/flag" '{}' /tmp/uc-4b.json)
refresh_csrf
log "flag-empty-reason" "$([ "$code" = "400" ] && echo OK || echo NOTE)" "code=$code body=$(head -c200 /tmp/uc-4b.json)"

# 4c. flag missing CSRF
code=$(call_no_csrf POST "/admin/solutions/sol-s-003/flag" '{"reason":"x"}' /tmp/uc-4c.json)
log "flag-no-csrf" "$([ "$code" = "403" ] && echo OK || echo NOTE)" "code=$code body=$(head -c200 /tmp/uc-4c.json)"

# 4d. flag without auth
BACKUP_COOKIE="$(cat "$COOKIE_JAR")"
: > "$COOKIE_JAR"
code=$(curl -sS -o /tmp/uc-4d.json -w '%{http_code}' -X POST "$BASE/admin/solutions/sol-s-003/flag" -H 'Content-Type: application/json' --data-binary '{"reason":"x"}')
printf "%s" "$BACKUP_COOKIE" > "$COOKIE_JAR"
log "flag-no-auth" "$([ "$code" = "401" -o "$code" = "403" ] && echo OK || echo NOTE)" "code=$code body=$(head -c200 /tmp/uc-4d.json)"

# ===== 5. POST /admin/solutions/{id}/unflag =====
echo "$SEP 5. POST /admin/solutions/sol-s-002/unflag $SEP"
code=$(call POST "/admin/solutions/sol-s-002/unflag" "" /tmp/uc-5.json)
refresh_csrf
log "unflag" "$([ "$code" = "200" ] && echo OK || echo FAIL)" "code=$code newCsrfLen=${#CSRF}"
python3 -c "import json; d=json.load(open('/tmp/uc-5.json')); v=d.get('data') or {}; print('isFlagged=',v.get('isFlagged'),'flaggedReason=',v.get('flaggedReason'),'flaggedAt=',v.get('flaggedAt'))"

# 5b. unflag when not flagged
code=$(call POST "/admin/solutions/sol-s-001/unflag" "" /tmp/uc-5b.json)
refresh_csrf
log "unflag-when-clean" "$([ "$code" = "200" ] && echo OK || echo NOTE)" "code=$code body=$(head -c200 /tmp/uc-5b.json)"

# ===== 6. POST /admin/solutions/bulk =====
echo "$SEP 6a. bulk action=publish $SEP"
code=$(call POST "/admin/solutions/bulk" '{"ids":["sol-s-001","sol-s-002"],"action":"publish"}' /tmp/uc-6a.json)
refresh_csrf
log "bulk-publish" "$([ "$code" = "200" ] && echo OK || echo FAIL)" "code=$code"
cat /tmp/uc-6a.json; echo

# 6b. bulk unpublish
code=$(call POST "/admin/solutions/bulk" '{"ids":["sol-s-001"],"action":"unpublish"}' /tmp/uc-6b.json)
refresh_csrf
log "bulk-unpublish" "$([ "$code" = "200" ] && echo OK || echo FAIL)" "code=$code"
cat /tmp/uc-6b.json; echo

# 6c. bulk unflag (after re-flagging)
code=$(call POST "/admin/solutions/sol-s-002/flag" '{"reason":"bulk test"}' /tmp/uc-6c0.json)
refresh_csrf
code=$(call POST "/admin/solutions/bulk" '{"ids":["sol-s-002"],"action":"unflag"}' /tmp/uc-6c.json)
refresh_csrf
log "bulk-unflag" "$([ "$code" = "200" ] && echo OK || echo FAIL)" "code=$code"
cat /tmp/uc-6c.json; echo

# 6d. bulk action=flag  (BUG-Q2: should now be 400, not 200 + per-row failure)
code=$(call POST "/admin/solutions/bulk" '{"ids":["sol-s-002"],"action":"flag"}' /tmp/uc-6d.json)
refresh_csrf
log "bulk-flag-unsupported" "$([ "$code" = "400" ] && echo OK || echo FAIL)" "code=$code"
cat /tmp/uc-6d.json; echo

# 6e. bulk action=garbage  (BUG-Q3: should now be 400, not 200 + per-row failure)
code=$(call POST "/admin/solutions/bulk" '{"ids":["sol-s-002"],"action":"dropdb"}' /tmp/uc-6e.json)
refresh_csrf
log "bulk-garbage-action" "$([ "$code" = "400" ] && echo OK || echo FAIL)" "code=$code"
cat /tmp/uc-6e.json; echo

# 6f. bulk empty ids (validation)
code=$(call POST "/admin/solutions/bulk" '{"ids":[],"action":"publish"}' /tmp/uc-6f.json)
refresh_csrf
log "bulk-empty-ids" "$([ "$code" = "400" ] && echo OK || echo NOTE)" "code=$code body=$(head -c200 /tmp/uc-6f.json)"

# 6g. bulk with one bad id  (BUG-Q4: existing → success:true, missing → success:false)
code=$(call POST "/admin/solutions/bulk" '{"ids":["sol-s-003","sol-NOPE-999"],"action":"publish"}' /tmp/uc-6g.json)
refresh_csrf
log "bulk-mixed-ids" "$([ "$code" = "200" ] && echo OK || echo FAIL)" "code=$code"
python3 -c "
import json
d=json.load(open('/tmp/uc-6g.json'))
arr=d.get('data',[])
...
"
cat /tmp/uc-6g.json; echo

# 6h. bulk delete
code=$(call POST "/admin/solutions/bulk" '{"ids":["sol-s-005"],"action":"delete"}' /tmp/uc-6h.json)
refresh_csrf
log "bulk-delete" "$([ "$code" = "200" ] && echo OK || echo FAIL)" "code=$code"
cat /tmp/uc-6h.json; echo

# ===== 7. DELETE /admin/solutions/{id} =====
echo "$SEP 7. DELETE /admin/solutions/sol-s-006 $SEP"
code=$(call DELETE "/admin/solutions/sol-s-006" "" /tmp/uc-7.json)
log "delete" "$([ "$code" = "200" ] && echo OK || echo FAIL)" "code=$code body=$(head -c200 /tmp/uc-7.json)"
...
code=$(call DELETE "/admin/solutions/sol-NOPE-XXX" "" /tmp/uc-7c.json)
log "delete-missing" "$([ "$code" = "404" -o "$code" = "50001" ] && echo OK || echo FAIL)" "code=$code body=$(head -c200 /tmp/uc-7c.json)"

# 7d. delete without CSRF
code=$(call_no_csrf DELETE "/admin/solutions/sol-s-001" "" /tmp/uc-7d.json)
log "delete-no-csrf" "$([ "$code" = "403" ] && echo OK || echo NOTE)" "code=$code body=$(head -c200 /tmp/uc-7d.json)"

# 7e. delete-no-csrf-no-cookie
: > "$COOKIE_JAR"
code=$(curl -sS -o /tmp/uc-7e.json -w '%{http_code}' -X DELETE "$BASE/admin/solutions/sol-s-001")
printf "%s" "$BACKUP_COOKIE" > "$COOKIE_JAR"
log "delete-no-auth" "$([ "$code" = "401" -o "$code" = "403" ] && echo OK || echo NOTE)" "code=$code body=$(head -c200 /tmp/uc-7e.json)"

# 7f. delete the just-deleted (should be 404 since MyBatis-Plus @TableLogic excludes)
code=$(call DELETE "/admin/solutions/sol-s-006" "" /tmp/uc-7f.json)
log "delete-already-deleted" "$([ "$code" = "404" ] && grep -q '"code":50401' /tmp/uc-7f.json && echo OK || echo NOTE)" "code=$code body=$(head -c200 /tmp/uc-7f.json)"

# ===== X. Anonymous probes =====
echo "$SEP X. anonymous probes $SEP"
for ep in "/admin/solutions" "/admin/solutions/flagged" "/admin/solutions/sol-s-001"; do
  code=$(curl -sS -o /tmp/uc-anon.json -w '%{http_code}' "$BASE$ep")
...
code=$(call GET "/auth/me" "" /tmp/uc-me.json)
log "auth-me" "$([ "$code" = "200" ] && echo OK || echo FAIL)" "code=$code"
# /auth/me returns data.user.role (nested) — not data.role
ROLE=$(python3 -c "import json; d=json.load(open('/tmp/uc-me.json')); print(d.get('data',{}).get('user',{}).get('role',''))" 2>/dev/null || echo "")
...
for i in $(seq 1 35); do
  ...
  rc=$(call POST "/admin/solutions/${RL_TARGET}/${ep}" "$payload" /tmp/uc-rl-$i.json 2>/dev/null)
