...
public class CommandReceiptExecutor {
    ...
    private final NotificationCommandReceiptMapper receiptMapper;
    ...
    public <T> RpcResult<T> execute(
        ...
        NotificationCommandReceiptEntity claim = newClaim(service, operation, command, fingerprint, traceId);
        ...
        if (keyedResult.success()) {
            try {
                ...
                if (receiptMapper.markSuccess(claim.getId(), payload) != 1) {
                    throw new IllegalStateException("Unable to finalize Notification command receipt");
                }
            } catch (Exception e) {
                throw new IllegalStateException("Unable to serialize Notification command receipt", e);
            }
        } else {
    ...
    private NotificationCommandReceiptEntity newClaim(
            String service,
            String operation,
            WriteCommand command,
            String requestFingerprint,
            String traceId) {
        NotificationCommandReceiptEntity receipt = new NotificationCommandReceiptEntity();
    ...
    private <T> RpcResult<T> replay(
            NotificationCommandReceiptEntity existing,
            String fingerprint,
            Class<T> resultType,
            String traceId,
            String key) {
        ...
        try {
            ...
            throw new IllegalStateException("Unable to replay Notification command receipt", e);
        }
    }
