...
| Final route, DB grant, Compose, security, concurrency and rollback gate | SPLIT-005 | PENDING — focused/module/integration/verify and frontend checks pass in an isolated environment; official quick is blocked by the pre-existing MySQL 1045 credential mismatch, all-IT is blocked by unchanged auth `AccountManagementTransactionalIT.deleteAccountSoftDelete`, and compatibility/grant retirement remains rollback-gated |
...
| Submission + judge outbox and verdict fence + result outbox stay locally strong-consistent | SPLIT-003 | PASS for transaction/fence/outbox and dispatcher evidence; isolated services `test`/`verify` pass, while the unrelated unchanged auth IT failure keeps SPLIT-005 open |
