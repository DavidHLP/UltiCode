...
| Submission aggregate and judge/result outboxes have one Owner/writer | SPLIT-003, SPLIT-004, CONTRACT-007 | PARTIAL — default local/compat path is restored with one App writer/fence seam, owner-local path has a real Submission reaper and outbox writer, and disposable schema/checksum/grant rollback is PASS; production route remains local until release observation |
