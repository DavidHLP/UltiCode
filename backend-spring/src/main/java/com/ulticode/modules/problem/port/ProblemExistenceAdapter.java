...
/**
 * Production adapter for {@link ProblemExistencePort}. Owns the
 * read-check-write cycle for the {@code has_solution} column so the
 * solution write path stops mutating the {@link Problem} entity
 * directly across the module seam.
 *
 * <p><b>Non-throwing contract</b>: missing problem rows are silent
 * no-ops for {@link #markHasSolution(Long, boolean)} (matches the
 * previous inline defensive null check in
 * {@code SolutionServiceImpl.delete(...)}).
 *
 * @author ulticode
 */
@Component
