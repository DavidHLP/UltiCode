#!/bin/bash
# C++ code runner for the sandbox
# Compiles and executes C++ code with provided inputs

set -e

TIME_LIMIT_MS="${TIME_LIMIT_MS:-10000}"
INPUT_FILE="${INPUT_FILE:-/sandbox/input/args.json}"
CODE_FILE="${CODE_FILE:-/sandbox/code/solution.cpp}"
...
# Compile C++ code with optimizations and security flags
COMPILE_START=$(date +%s%3N)
if ! g++ -std=c++17 -O2 -o "$OUTPUT_BIN" "$CODE_FILE" \
    -static \
    -std=c++17 \
    -O2 \
    -fno-exceptions \
    -fno-rtti \
    2>compile_error.txt; then
...
if [ -f "$INPUT_FILE" ]; then
    # Read args and format for stdin
