...
class AppAuditOutboxDispatcherTest {

    @Mock
    private AppAuditOutboxMapper outboxMapper;
    @Mock
    ...
    void dispatchesClaimedAuditRow() {
        ...
        verify(streamOperations).add(argThat(streamRecord ->
                IntegrationEventEnvelopeContract.APP_AUDIT_STREAM_KEY.equals(streamRecord.getStream())));
        verify(outboxMapper).markDelivered(eq("app-audit-1"), anyString());
    }
    ...
    void retriesWhenRedisPublishFails() {
        ...
        verify(outboxMapper).markRetry(
                eq("app-audit-2"), anyString(), eq("redis down"), eq(5), eq(30));
