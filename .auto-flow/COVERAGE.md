...
| Auth/Admin/App/Notification isolation preparation | ARCH-002 | BLOCKED — local TEST-TARGET/DEV-LOCAL evidence is retained, but external target authority, users/profile responsibility sign-off, physical cutover and rollback evidence remain missing |
| App dual-track cleanup | ARCH-003 | BLOCKED — local monitoring/quiesce/observation/compatibility rehearsals are retained, but production stability, deployment authority, observation window, rollback and retirement evidence remain missing |
...
| --- | --- | --- |
| 1.1 设计并实现特权 owner migration job | ARCH-002-001 | PASS — local migration job and disposable preflight evidence |
| 1.2 获取真实数据库权限与 cutover window | ARCH-002-002 | BLOCKED — local TEST-TARGET grant rehearsal retained; external target authority/sign-off is not established |
| 1.3 完成 users/profile Owner 职责 | ARCH-002-003 | BLOCKED — local 12/12 parity and idempotent backfill retained; external responsibility/cutover sign-off missing |
| 1.4 解除 Auth MySQL 1044 验证门禁 | ARCH-002-004 | BLOCKED — local privileged validation and runtime denial retained; external target evidence missing |
| 1.5 执行物理隔离切换测试 | ARCH-002-005 | BLOCKED — disposable/local cutover and rollback retained; external physical cutover evidence missing |
| 2.1 建立 remote-route 稳定性监控 | ARCH-003-001 | BLOCKED — local baseline retained; external production stability window/deployment authority missing |
| 2.2 获取全部写入者静默确认 | ARCH-003-002 | BLOCKED — local signed rehearsal retained; external all-writer quiesce evidence missing |
| 2.3 制定并执行观察期与 rollback 演练 | ARCH-003-003 | BLOCKED — local fault rehearsal retained; external observation/rollback evidence missing |
| 2.4 部署测试环境监控与切换机制 | ARCH-003-004 | BLOCKED — local Compose/monitoring checks retained; external deployment authority missing |
| 2.5 完成兼容退役功能与性能测试 | ARCH-003-005 | BLOCKED — local compatibility scan/rehearsal retained; external retirement evidence missing |
| 实施限制：只操作当前 TEST-TARGET，不制造 production 证据 | ARCH-002-002..ARCH-003-005 | PASS — local evidence explicitly bounded; external acceptance remains open |
| 验证标准：focused tests、Maven、diff、bash、YAML | ARCH-007 | IN PROGRESS — local checks retained; final gate awaits external ARCH-002/003 evidence |
| 关闭条件：所有 blocker 有 Acceptance/Validation/Review/Rollback/Evidence | ARCH-007 | BLOCKED — external evidence is still absent |

Status: `ARCH-002` and `ARCH-003` are `blocked`; local DEV-LOCAL/TEST-TARGET evidence is retained and must not be promoted to external acceptance. ARCH-006/ARCH-007 remain in progress pending the external gates.
