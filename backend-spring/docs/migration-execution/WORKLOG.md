...
## 2026-07-28 P3-BURNDOWN-001 + P3-GATE — Phase 3 closed

- **P3-BURNDOWN-001**: burned down the 8 frozen admin foreign-mapper write violations recorded in the p3_owner_001_f freeze store (a550ee6a).
  - New `TestCaseOwnerPort` + `DefaultTestCaseOwnerPort` in `com.ulticode.modules.problem.port` — owner-only write surface for `test_cases` rows. Routed the 5 `AdminTestCaseService` TestCaseMapper write calls (insert, updateById, deleteById, delete-by-problem, reorder updateById) through it; TestCaseMapper is now read-only inside the admin module.
  - Extended `ProblemOwnerPort` with `insertImportedProblem` / `applyImportedUpdate`; routed the 3 `ProblemImportServiceImpl` ProblemMapper write calls through them. Import row defaults (status "todo", isPremium/isPublished false, hasSolution/isFlagged/isDeleted false, version 1) and PartialUpdate null-skip / blank-skip semantics moved into `DefaultProblemOwnerPort`.
  - AC1 wording revised post-impl: test_cases rows use a sibling `TestCaseOwnerPort` (problem-domain) rather than overloading `ProblemOwnerPort`, because test_cases is a distinct table and a sibling port preserves module cohesion.
  - Refroze `p3_owner_001_f` with an empty store (da138919, 0 bytes; old a550ee6a removed). `OwnerBoundaryArchTest` 8/8 green.
  - Commit: 51af2a0.
- **P3-AUDIT-001 follow-up (found during gate IT walk)**: `AuditOutboxMapper` lived in `com.ulticode.modules.admin.outbox` but the app's `@MapperScan` only covers `**.mapper` packages. Once an explicit `@MapperScan` is present, bare `@Mapper` annotations outside the scan path are not auto-registered (MyBatis-Spring gotcha), so the mapper bean was never created and `DefaultAuditSinkAdapter` -> `AuditAspect` wiring threw `NoSuchBeanDefinitionException` at full-context startup. The unit suite (fully mocked) masked this; the full-context `*IT` tests were the canary. Moved the mapper to `admin.outbox.mapper` per the convention documented in `MapperConfig` (cf. queue.outbox.mapper, notification.ledger.mapper). Commit: b9c66db.
- **P3-GATE closed**. AC1 (zero write violations), AC2 (Auth/Profile seam rules green), AC3 (single-direction graph, reverse-import walk clean), AC5 (ArchUnit 8/8) all PASS. AC4: `./mvnw verify -B` = 1863/0/0. `*IT` suite = 65 pass / 13 fail, **all 13 environment/infrastructure-only**:
  - RateLimitIT + SubmissionServiceImplIT: Testcontainers Redis has no password but Redisson sends AUTH (`ERR AUTH called without any password configured for the default user`).
  - SandboxNamespaceIsolationIT + SandboxForkE2EIT: need root/namespace privilege + missing `docker/sandbox/seccomp-profile.json` fixture.
