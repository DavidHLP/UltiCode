...
public class AuthSingleHopArchTest {
    ...
    private static final ArchCondition<JavaClass> MUST_NOT_DECLARE_DUBBO_REFERENCE_FIELD =
            new ArchCondition<>("not declare a field annotated with @DubboReference") {
                @Override
    ...
    static final ArchRule DUBBO_PROVIDER_MUST_NOT_ALSO_BE_CONSUMER =
            classes()
                    ...
                    .should(MUST_NOT_DECLARE_DUBBO_REFERENCE_FIELD)
    ...
    /**
     * HTTP adapters may depend on request/response DTOs and capability
     * interfaces, but never on Auth persistence packages or password-bearing
     * account records.
     */
    @ArchTest
    static final ArchRule WEB_ADAPTERS_MUST_NOT_DEPEND_ON_AUTH_STORAGE =
            noClasses()
                    .that().resideInAPackage("com.ulticode.auth.adapter.in.web..")
                    .should().dependOnClassesThat().resideInAnyPackage(
                            "com.ulticode.auth.account..",
                            "com.ulticode.auth.permission.entity..",
                            "com.ulticode.auth.permission.mapper..")
                    .because("HTTP adapters must use capability interfaces and "
                            + "transport-safe value objects, not Auth storage seams.");

    /**
     * Capability workflows remain HTTP-neutral. Servlet mutation belongs to
     * inbound web adapters and their cookie/error infrastructure.
     */
    @ArchTest
    static final ArchRule AUTH_CAPABILITY_WORKFLOWS_MUST_BE_HTTP_NEUTRAL =
            noClasses()
                    .that().resideInAPackage("com.ulticode.auth.service..")
                    .should().dependOnClassesThat().resideInAnyPackage(
                            "jakarta.servlet..",
                            "javax.servlet..")
                    .because("Auth capability workflows must not carry servlet "
                            + "request/response dependencies across the seam.");

    @Test
    void providerConsumerRuleChecksReferenceFields() {
        var imported = new ClassFileImporter().importClasses(ProviderConsumerFixture.class);

        assertThatThrownBy(() -> DUBBO_PROVIDER_MUST_NOT_ALSO_BE_CONSUMER.check(imported))
                .isInstanceOf(AssertionError.class)
                .hasMessageContaining("@DubboReference field");
    }

    @DubboService
    static class ProviderConsumerFixture {
        @DubboReference
