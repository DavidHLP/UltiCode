...
public class SubmissionRejudgeService {
    ...
    @Transactional
    public RejudgeOutcome rejudge(String submissionId) {
        Submission submission = submissionMapper.selectById(submissionId);
        if (submission == null) {
            return failure(AppErrorCode.CONTENT_NOT_FOUND, "Submission not found");
        }
        if (createdOutboxMapper.findLatestBySubmissionId(submissionId) != null) {
            return failure(AppErrorCode.CONTENT_STATE_CONFLICT,
        ...
        if (current == SubmissionStatus.JUDGING) {
            if (submissionMapper.requestJudgingRejudge(submissionId, generation) != 1) {
                return failure(AppErrorCode.VERSION_CONFLICT,
    ...
    private RejudgeOutcome success(Submission submission, String status) {
        int retryCount = submission == null || submission.getRetryCount() == null
                ? 0 : submission.getRetryCount();
        return new RejudgeOutcome.Initiated(
    ...
    private static RejudgeOutcome failure(AppErrorCode code, String message) {
        return new RejudgeOutcome.Rejected(code, message);
    }
}
