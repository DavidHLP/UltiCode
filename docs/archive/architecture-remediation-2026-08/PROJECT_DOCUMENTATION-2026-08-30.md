...
- 旧单体拓扑仅保留为迁移历史；当前实现以 `services/pom.xml`、各 Owner/Worker `application.yml`、`docker/docker-compose.prod.yml` 和 `ecosystem.config.cjs` 为准；
...
- 四个 owner 分别由各自的安全配置组装无状态安全链，CSRF filter 复用 `platform/web-security`；JWT 认证统一由 `com.ulticode.websecurity.jwt` 的共享 filter/verifier/JWKS cache 提供，Auth 通过 `AuthAccessTokenVerifier` 接入，Admin、App、Notification 通过 `JwtResourceServerConfiguration` 接入。`AuthSecurityConfig`、`AdminSecurityConfig`、`AppSecurityConfig`、`NotificationSecurityConfig` 只保留 owner 路由与 filter-chain 组合。
...
`P3-IDENTITY-001` 为 Auth、Admin、App、Submission、Notification、Judge 的 Dubbo Triple 配置启用生产可选的双向 TLS，并在 `backend-rpc-resilience` 中以 provider/consumer SPI filter 强制 peer certificate identity。Provider 的 caller SAN 必须命中目标 Owner 的 allowlist；Consumer 同时根据本地 `@DubboReference` group 验证服务端 SAN 与目标 identity，避免仅依赖 CA 信任。证书 SAN 必须是精确的 SPIFFE URI `spiffe://ulticode/service/<service>`（或等价的内部 DNS SAN）；目标 Owner 的 allowlist 与 delegation audience 一致：Auth 接受 Admin/App/Notification/Submission，App 接受 Admin/Submission/Judge，Submission 接受 Admin/App/Judge，Notification 接受 Admin，Judge 接受 App，Admin 当前没有业务 consumer。缺少 TLS session、证书无效、过期、SAN 不匹配或 caller 不在 audience 内均 fail closed。

生产证书目录只通过 `DUBBO_MTLS_CERT_DIR` 注入，Compose 以 `:ro` 挂载到 `/run/secrets/dubbo`；`trusted-callers.pem`/`trusted-services.pem` 是 CA bundles，provider/consumer filter 再次校验 SAN、caller 与 target 矩阵。轮换需外部 secret store 先重叠新旧 CA/leaf、滚动重建并完成拒绝旧材料的验证；本仓库只提供 fixture/contract，不保存或轮换真实生产密钥。
#### Service network reachability

`P3-NET-001` 将 Compose 网络按实际调用图拆分为 `edge`、`sql`、`cache`、`registry`、`search`、`egress`、`observability` 与五条点对点 Dubbo RPC 网络。Console/Management 只加入 `edge`；MySQL 只加入 `sql`，Redis 只加入 `cache`，Nacos 加入 `sql`/`registry`/受控 `egress`，MeiliSearch 只加入 `search`。Auth/Admin/App/Notification 的 HTTP ingress 只经过 `edge`，Judge 不加入 `edge`/`sql`/`search`；Dubbo caller 只与其真实 target 共享对应的 `rpc-*` 网络。除专用于外部 SMTP/OAuth/S3/OTLP/注册中心 peer 的 `egress` 外，所有网络在 base/prod 默认 `internal: true`，backend 不发布端口。

开发覆盖层只将 `sql`、`cache`、`registry`、`search` 设为非 internal 以保留现有 loopback infrastructure mappings；它不改变 production Compose。需要访问外部 SMTP/OAuth/S3/OTLP 或 Nacos peer 的 workload 使用受控 `egress` 网络，生产防火墙仍必须按目的地址 allowlist。可选 observability overlay 的六个组件和所有 backend worker 只通过 `observability` 互通。`scripts/test/network-reachability-contract.sh` 同时检查静态成员/禁止路径并运行 disposable Docker allow/deny network drill；真实生产 firewall、DNS、Nacos peer 和 ingress authority 仍需外部验证。
...
- `P3-IDENTITY-001` 将 Dubbo Triple transport identity 与 end-user delegation 分开：`backend-rpc-resilience` 注册 provider/consumer `workload-mtls` filter。生产必须设置 `DUBBO_PROTOCOL_SSL_ENABLED=true`、`DUBBO_SSL_DEFAULT=true`，每个 `backend-auth/admin/app/submission/notification/judge` 从 `${DUBBO_MTLS_CERT_DIR}/<service>` 只读挂载 `identity.crt`、`identity.key`、`trusted-callers.pem` 与 `trusted-services.pem`。Provider 只接受来自 TLS peer certificate 的单一 `spiffe://ulticode/service/<service>` 或 `<service>.ulticode.internal` SAN，并按实际 Dubbo delegation audience 矩阵拒绝未知、过期、错误 SAN 与未授权 caller；Consumer 依据本地 `@DubboReference` group 验证服务端 SAN 与 target。两侧均不读取 invocation attachment、`remote.application` 或 request DTO 作为 transport identity。
- 证书/CA rotation 由外部 secret store 完成：先在对应 trust bundles 中重叠旧/新材料并重建 workload，再移除旧材料；仓库不提交真实证书或私钥。`scripts/test/dubbo-mtls-contract.sh` 覆盖六个 Triple YAML/生产挂载、有效证书、未知/过期/错误 SAN/未授权证书和 TLS handshake；真实 Nacos/Dubbo 注册、secret rotation 与生产 rollout 仍是外部证据。
...
- `P3-HA-001` 的非默认 `docker/docker-compose.ha.yml` profile `ha` 是可审计的 stateful reference profile，不是默认生产拓扑：MySQL primary 开启 GTID/binlog 并提供 `mysql-replica`，但复制初始化、提升、切换 `DB_HOST` 和回切必须由外部运维 authority 执行；它不宣称无感自动故障转移，也不替代 `owner-backup-restore.sh` 的加密备份与 restore drill。
...
- Dubbo Triple workload mTLS 仅在 production override 开启：六个带 Triple 的 workload 各自从 `${DUBBO_MTLS_CERT_DIR}/<service>` 只读挂载独立 identity 与 trust bundle，`DUBBO_MTLS_SERVICE_IDENTITY` 固定对应 `DUBBO_APPLICATION_NAME`，`DUBBO_MTLS_ALLOWED_CALLERS` 固定对应 delegation audience；provider `workload-mtls` filter 从 TLS session 读取 caller X509 SAN，consumer 同时从本地 `@DubboReference` group 验证目标服务 SAN，缺少 session、过期/未知/错误 SAN 或未授权 caller/target 均拒绝。轮换必须由外部 secret store 先重叠新旧材料、滚动重建、验证旧证书拒绝后再移除旧材料；`scripts/test/dubbo-mtls-contract.sh` 是无真实凭据的本地/CI proof。
- MeiliSearch 继续按单节点 derived read model 处理；本仓库不承诺 active-active HA。节点丢失时先恢复 Owner 数据与 worker，再清理 `search:doc-version:{index}` 版本账本、按 §11.5 watermark/backfill 重建并观察索引计数；事件 outbox/PEL 负责重放，不能把 MeiliSearch 副本当作业务数据备份。可执行检查：`HA_COMPOSE_ENV_FILE=... bash scripts/test/ha-profile-contract.sh`；授权一次性 Docker 环境另设 `HA_RECONNECT_DRILL=1` 验证 Redis 重启后的连接与持久状态。
...
- Redis ACL 采用 `default off`、按 Owner 的精确命令与 key/channel pattern；唯一共享 key 是 `stream:integration` 事件流。业务用户禁止 `FLUSHDB`、`FLUSHALL`、`CONFIG`、`SHUTDOWN`、`MODULE`、`DEBUG`，健康用户 `ulticode-health` 仅允许 `PING`，`ulticode-replication` 与 `ulticode-sentinel` 仅允许 HA 控制面命令，运维测试用户与健康身份分离。所有这些凭据由 `redis-acl-rotation.sh` 原子轮换，当前与 previous hash 可短暂重叠。
...
- 无 HTTP 面的 Worker（judge/search）由心跳组件刷新 `/tmp` 下的就绪标记文件，compose healthcheck 校验标记新鲜度（search 同时校验 Redis + MeiliSearch，judge 通过远程/rootless Docker TLS `docker info`、沙箱镜像和 heartbeat 检查能力）。
...
- `backend-judge` 是独立 Maven module/image；它只消费 Redis Streams 并通过 Problem/Submission owner contracts 读 facts、抢 lease、写 verdict。提交、`judge_outbox`、lease/fence、result outbox 的数据 Owner 目标态为 `backend-submission`。生产 Compose 只通过 `JUDGE_DOCKER_HOST` 连接部署拥有的 remote/rootless Docker daemon，使用 `DOCKER_TLS_VERIFY=1`、只读 `/run/secrets/judge-docker` client bundle 和共享 sandbox workspace；不挂载 docker.sock、不加入 `DOCKER_GID`，不发布 HTTP/Dubbo 到公网。开发需显式叠加 `docker/docker-compose.judge-dev.yml --profile judge-socket`，该 profile 才允许 loopback/ disposable socket。
...
- 运行拓扑：`docker/docker-compose*.yml`、`console/nginx.conf`、`management/nginx.conf`、`scripts/dev/**`。
