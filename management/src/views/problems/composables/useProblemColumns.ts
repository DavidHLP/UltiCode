  ...
  return [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return h('span', { class: 'text-muted-foreground text-xs font-mono' }, String(id ?? '').slice(0, 8))
      },
    },
    {
      accessorKey: 'title',
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const variant = isSolved ? 'default' as const : isAttempted ? 'secondary' as const : 'outline' as const
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return h(Badge, { variant: isPublished ? 'default' : 'secondary' }, {
          default: () => [
            isPublished ? h(IconCheck, { class: 'mr-1 h-3 w-3' }) : h(IconEyeOff, { class: 'mr-1 h-3 w-3' }),
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return h('div', { class: 'flex items-center gap-1', title: `${t(statusKey)}${problem.flagReason ? `: ${problem.flagReason}` : ''}` }, [
          h(IconFlag, { class: ['h-4 w-4', colorClass, flagStatus === 'PENDING' ? 'animate-pulse-subtle' : ''].join(' ') }),
          h('span', { class: ['font-data text-[10px] uppercase', colorClass].join(' ') }, t(statusKey).slice(0, 3)),
        ])
      },
    },
    {
      ...
      cell: ({ row }) => h('span', { class: 'text-muted-foreground text-sm tabular-nums' }, (row.original.submissionCount || 0).toLocaleString()),
    },
    {
      accessorKey: 'tags',
    ...
    },
    {
      ...
      cell: ({ row }) => h('span', { class: 'text-muted-foreground text-sm' }, new Date(row.getValue('createdAt') as Date).toLocaleDateString()),
    },
    {
      id: 'actions',
          ...
          default: () => [
            ...
            h(DropdownMenuContent, { align: 'end' }, {
              default: () => [
                h(DropdownMenuSub, {}, {
                  default: () => [
                    h(DropdownMenuSubTrigger, { class: 'gap-2' }, { default: () => [h(IconEye, { class: 'h-4 w-4' }), t('common.view')] }),
