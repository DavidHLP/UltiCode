...
- **Sidebar-collapsed (icon-only) mode is out of scope** for the shared components. Both apps keep shadcn `SidebarMenuButton` + tooltip for that branch, so the shared layer only covers the expanded sidebar.
