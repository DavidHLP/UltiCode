-- P1-AUDIT-001: Auth-local audit outbox. Admin consumes committed rows as
-- AuditRecorded events; Auth never writes the Admin schema.
...
DROP PROCEDURE IF EXISTS `_revoke_auth_cross_owner_audit_grant`;
DELIMITER //
CREATE PROCEDURE `_revoke_auth_cross_owner_audit_grant`()
BEGIN
    DECLARE grant_count INT DEFAULT 0;
    SELECT COUNT(*) INTO grant_count
    FROM information_schema.table_privileges
    WHERE GRANTEE = '''auth_rw''@''%'''
      AND TABLE_SCHEMA = 'admin'
      AND TABLE_NAME = 'audit_outbox'
      AND PRIVILEGE_TYPE = 'INSERT';
    IF grant_count > 0 THEN
        SET @revoke_sql = 'REVOKE INSERT ON `admin`.`audit_outbox` FROM ''auth_rw''@''%''';
    ...
    IF grant_count > 0 THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'auth_rw still has a cross-owner audit_outbox INSERT grant';
    END IF;
END//
DELIMITER ;
CALL `_revoke_auth_cross_owner_audit_grant`();
DROP PROCEDURE IF EXISTS `_revoke_auth_cross_owner_audit_grant`;
