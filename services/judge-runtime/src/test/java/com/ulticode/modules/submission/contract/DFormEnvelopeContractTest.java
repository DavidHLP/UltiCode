...
class DFormEnvelopeContractTest {
    ...
    void decodeGoldenEnvelope() throws Exception {
        List<JudgeRunRequest.TestCase> testCases = buildTestCasesFromGoldenInput();
        String runId = "test-run";
        String userId = "test-user";

        List<JudgeRunResponse.RunCaseResult> actual =
                helper.parseDEnvelope(goldenEnvelopeJson, testCases, runId, userId);

        assertThat(actual).hasSize(3);

        JudgeRunResponse.RunCaseResult c1 = actual.get(0);
        ...
        JudgeRunResponse.RunCaseResult c3 = actual.get(2);
    ...
    void encodeBatchInputs_topLevelShape() throws Exception {
        List<JudgeRunRequest.TestCase> cases = buildTestCasesFromGoldenInput();
     ...
     */
    private List<JudgeRunRequest.TestCase> buildTestCasesFromGoldenInput() throws Exception {
        JsonNode root = MAPPER.readTree(goldenInputJson);
        List<JudgeRunRequest.TestCase> out = new ArrayList<>();
        ...
        while (it.hasNext()) {
            ...
            List<JudgeRunRequest.Input> inputs = new ArrayList<>();
            for (JsonNode in : c.get("inputs")) {
