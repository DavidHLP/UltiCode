...
capture_env_vars MIGRATION_DB_HOST MIGRATION_DB_PORT MIGRATION_DB_NAME MIGRATION_DB_USER \
  ...
  BOOTSTRAP_DELEGATION_PUBLIC_KEY BOOTSTRAP_DELEGATION_KEY_ID HEALTH_REDIS_PASSWORD \
...
verify_owner_accounts() {
  ...
  for owner in "${DEVSTACK_OWNER_MIGRATION_ORDER[@]}"; do
    ...
    docker exec -e MYSQL_PWD="$password" "$container" \
      ...
      --batch --skip-column-names -h 127.0.0.1 -P "${MIGRATION_MYSQL_CONTAINER_PORT:-3306}" "$database" \
      -e "SELECT 1" >/dev/null || {
...
if [[ "$SKIP_BOOTSTRAP" != true && "$DEV_SEED_USERS_ENABLED" == "true" ]]; then
  ...
  (
    ...
    DUBBO_REGISTRY_USERNAME="$AUTH_NACOS_USERNAME" \
    DUBBO_REGISTRY_PASSWORD="$AUTH_NACOS_PASSWORD" \
