...
/**
 * JWT authentication filter for the backend-admin resource server shell
 * (P7-RELOCATE). Mirrors backend-auth's
 * {@code com.ulticode.auth.security.jwt.JwtAuthenticationFilter} but uses
 * the admin-local {@link ResourceServerJwtVerifier} for offline token
 * validation with the shared JWT secret.
 *
 * <p>The admin shell excludes the App-owned security package, so each
 * context owns its own filter + verifier pair. Both sides validate with
 * the same secret issued by backend-auth.
 */
@Slf4j
@Component
