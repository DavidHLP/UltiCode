 ...
 * (is_virtual filter) fix.
 */
@ExtendWith(MockitoExtension.class)
@MockitoSettings(strictness = Strictness.LENIENT)
@DisplayName("RatingCalculationServiceImpl (P1-4, P1-5 fixes)")
class RatingCalculationServiceImplTest {
    ...
    void calculateAndUpdate_preloadsRatingsInOneQuery() {
        ...
        when(participantMapper.findByContestIdAndStatus(CONTEST_ID, "STARTED"))
    ...
    /** P1-4: virtual participants are filtered out of the rating calculation. */
    @Test
    @DisplayName("P1-4: virtual participants are excluded from rating update")
    void calculateAndUpdate_skipsVirtualParticipants() {
        ...
        when(participantMapper.findByContestIdAndStatus(CONTEST_ID, "STARTED"))
                .thenReturn(new java.util.ArrayList<>(List.of(real, virt)));
