...
async function initToastNotifier() {
  ...
  if (!toastNotifier) {
    try {
      ...
      console.warn("vue-sonner not available:", error);
    }
  }
...
export function setStoredLocale(locale: string): void {
  ...
  try {
    ...
    console.warn("Storage write failed, attempting fallback:", e);
...
/**
 * Remove stored locale from active storage
 *
 * @example
 * removeStoredLocale(); // Clear the stored locale preference
 */
export function removeStoredLocale(): void {
  ...
  try {
    if (storage instanceof Map) {
      storage.delete(LOCALE_STORAGE_KEY);
    } else {
      storage.removeItem(LOCALE_STORAGE_KEY);
    }
  } catch (e) {
    console.warn("Failed to remove locale from storage:", e);
  }
}

/**
 * Get current storage layer status (for debugging/UI feedback)
 *
 * @returns The active storage layer name
 *
 * @example
 * const status = getStorageStatus();
 * console.log('Using:', status); // "localStorage" | "sessionStorage" | "memory"
 */
export function getStorageStatus(): StorageLayer {
  initializeStorage();
  return activeStorageLayer;
}

/**
 * Manually re-check storage availability
 *
 * Call this when storage might become available (e.g., user changes
 * browser settings, exits private mode, etc.). Automatically notifies
 * user if storage recovers to localStorage.
 *
 * @example
 * // After user exits private browsing mode
 * recheckStorage();
 */
export function recheckStorage(): void {
  if (!storageInitialized) {
    initializeStorage();
    return;
  }
  ...
  if (newLayer !== previousLayer) {
    ...
    // Notify if storage recovered to localStorage
    if (newLayer === StorageLayer.LocalStorage) {
      const stored = getStoredLocale();
      const locale = (stored || getCurrentLocale()) as SupportedLocale;
      notifyStorageChange(previousLayer, newLayer, locale);
    }
  }
}
