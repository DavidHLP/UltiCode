...
user ulticode-auth on $(password_hashes AUTH_REDIS_PASSWORD) resetkeys ~csrf:* ~oauth:* ~rate-limit:* ~auth:* ~security:delegation:replay:* ~stream:integration resetchannels -@all $COMMON_COMMANDS (+xadd ~stream:auth-audit)
user ulticode-admin on $(password_hashes ADMIN_REDIS_PASSWORD) resetkeys ~rate-limit:* ~userStats:* ~contestRanking:* ~contest:* resetchannels -@all $COMMON_COMMANDS (+xreadgroup +xpending +xclaim +xack +xgroup|create +xinfo ~stream:app-audit) (+xreadgroup +xpending +xclaim +xack +xgroup|create +xinfo ~stream:auth-audit)
user ulticode-app on $(password_hashes APP_REDIS_PASSWORD) resetkeys ~rate-limit:* ~userStats:* ~contestRanking:* ~contest:* ~monitoring:* ~queue:* ~judge:* ~problem:* ~email_queue ~notification_queue ~security:delegation:replay:* ~stream:integration resetchannels &ulticode:ws:broadcast -@all $COMMON_COMMANDS $PUBSUB_COMMANDS +info (+xadd ~stream:app-audit) (+exists ~blacklist:token:*)
...
user ulticode-notification on $(password_hashes NOTIFICATION_REDIS_PASSWORD) resetkeys ~stream:integration ~rate-limit:email:* ~rate-limit:notification:* ~poison:* ~notification:* ~security:delegation:replay:* resetchannels &ulticode:ws:broadcast -@all $COMMON_COMMANDS $PUBSUB_COMMANDS
