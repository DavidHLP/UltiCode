 ...
 * place and the write service does not duplicate it.
 *
 * <p>P7-RELOCATE-FORUM-001: dead {@code VoteService} field removed;
...
public class ForumPostServiceImpl implements ForumPostService {
    ...
    public ForumPostVO createPost(CreatePostDTO dto, String userId) {
        ...
        if (community == null) throw new BusinessException(ForumErrorCode.FORUM_COMMUNITY_NOT_FOUND);
        if ("PRIVATE".equals(community.getVisibility()) && !memberMapper.isMember(dto.getCommunityId(), userId))
            throw new BusinessException(ForumErrorCode.FORUM_COMMUNITY_RESTRICTED);
    ...
    public ForumPostVO updatePost(String id, UpdatePostDTO dto, String userId) {
        ...
        if (!post.getUserId().equals(userId)) throw new BusinessException(ForumErrorCode.FORUM_CANNOT_EDIT_POST);
        if (Boolean.TRUE.equals(post.getIsLocked())) throw new BusinessException(ForumErrorCode.FORUM_POST_LOCKED);
