...
-- 1.2 Insert default scoring rules (ignore if exists)
INSERT IGNORE INTO `contest_scoring_rules` (`id`, `name`, `description`, `base_score_per_problem`, `time_bonus_per_minute`, `wrong_answer_penalty`, `first_solve_bonus`, `is_default`) VALUES
...
-- Note: Some columns may already exist in baseline, so we handle them carefully
-- ============================================

-- 2.1 Extend contests table (add columns that don't exist in baseline)
ALTER TABLE `contests` ADD COLUMN `end_time` DATETIME(3) NULL;
ALTER TABLE `contests` ADD COLUMN `registration_start` DATETIME(3) NULL;
ALTER TABLE `contests` ADD COLUMN `registration_end` DATETIME(3) NULL;
ALTER TABLE `contests` ADD COLUMN `freeze_time` DATETIME(3) NULL;
ALTER TABLE `contests` ADD COLUMN `is_virtual` BOOLEAN NOT NULL DEFAULT FALSE;
ALTER TABLE `contests` ADD COLUMN `max_participants` INT NULL;
ALTER TABLE `contests` ADD COLUMN `scoring_rule_id` VARCHAR(40) NULL;
ALTER TABLE `contests` ADD COLUMN `submission_count` INT NOT NULL DEFAULT 0;
...
-- 2.4 Extend contest_participants table (only add columns not in baseline)
-- Note: is_virtual, total_score, total_attempts already exist in baseline as is_virtual, total_score, total_attempts
ALTER TABLE `contest_participants` ADD COLUMN `checked_in_at` DATETIME(3) NULL;
ALTER TABLE `contest_participants` ADD COLUMN `total_time` INT NOT NULL DEFAULT 0;
-- attempt_count is total_attempts in baseline, but we add it with different name
ALTER TABLE `contest_participants` ADD COLUMN `attempt_count` INT NOT NULL DEFAULT 0;

-- 2.5 Extend contest_problems table
ALTER TABLE `contest_problems` ADD COLUMN `label` VARCHAR(10) NULL;
ALTER TABLE `contest_problems` ADD COLUMN `base_score` INT NULL;
ALTER TABLE `contest_problems` ADD COLUMN `time_bonus` INT NULL;
...
ALTER TABLE `contest_problem_results` ADD COLUMN `time_spent` INT NOT NULL DEFAULT 0;
ALTER TABLE `contest_problem_results` ADD COLUMN `time_bonus` INT NOT NULL DEFAULT 0;
ALTER TABLE `contest_problem_results` ADD COLUMN `is_first_solve` BOOLEAN NOT NULL DEFAULT FALSE;

-- 2.8 Extend contest_rankings table
ALTER TABLE `contest_rankings` ADD COLUMN `is_frozen` BOOLEAN NOT NULL DEFAULT FALSE;
