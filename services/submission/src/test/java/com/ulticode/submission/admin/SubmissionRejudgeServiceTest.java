...
class SubmissionRejudgeServiceTest {
    ...
    @Test
    void missingSubmissionIsRejectedWithoutSideEffects() {
        RejudgeOutcome result = service.rejudge("missing");

        assertThat(result).isEqualTo(new RejudgeOutcome.Rejected(
                AppErrorCode.CONTENT_NOT_FOUND, "Submission not found"));
        verify(judgeOutboxMapper, never()).insert(any(JudgeOutboxRecord.class));
    }

    @Test
    void contestSubmissionIsRejectedWithoutSideEffects() {
        ...
        RejudgeOutcome result = service.rejudge("sub-1");

        assertThat(result).isEqualTo(new RejudgeOutcome.Rejected(
                AppErrorCode.CONTENT_STATE_CONFLICT,
                "Contest submissions cannot be rejudged until replacement scoring is supported"));
        verify(judgeOutboxMapper, never()).insert(any(JudgeOutboxRecord.class));
    }

    @Test
    void terminalSubmissionInitiatesRejudgeAndWritesOneRealJudgeTask() {
        ...
        assertThat(result).isEqualTo(new RejudgeOutcome.Initiated(
                "sub-1",
                "Pending",
    ...
    @Test
    void concurrentTerminalChangeIsRejectedWithoutJudgeTask() {
        ...
        assertThat(result).isEqualTo(new RejudgeOutcome.Rejected(
                AppErrorCode.VERSION_CONFLICT, "Submission changed concurrently"));
        verify(judgeOutboxMapper, never()).insert(any(JudgeOutboxRecord.class));
    }

    @Test
    void judgingRequestInitiatesLeaseExpiryWithoutDuplicateTask() {
    ...
    @Test
    void pendingSubmissionIsRejectedWithoutJudgeTask() {
